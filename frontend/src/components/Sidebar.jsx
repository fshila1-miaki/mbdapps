...
const Sidebar = ({ open, onClose }) => {
  ...
  return (
    <>
      ...
      <aside data-testid="sidebar" data-collapsed={collapsed} className={`${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed left-0 top-0 z-50 ${width} h-screen bg-[#09090B] text-white flex flex-col transition-[width,transform] duration-200 ease-in-out`}>
        {/* Collapse toggle */}
        <button data-testid="sidebar-collapse-toggle" onClick={() => setCollapsed(!collapsed)}
          className="hidden lg:flex absolute -right-3 top-20 w-6 h-6 bg-white text-[#09090B] border border-slate-200 rounded-full items-center justify-center shadow-sm hover:bg-slate-50 z-10">
        ...
        <div className="h-16 px-3 flex items-center justify-between border-b border-white/10">
          <Link to={user.role === "admin" ? "/admin" : "/dashboard"} className="flex items-center gap-2" onClick={onClose}>
            <OrbitMark size={30} className="text-white shrink-0" />
            {!collapsed && <span className="font-bold text-lg tracking-tight" style={{ fontFamily: "'Outfit', sans-serif" }}>Orbit</span>}
          </Link>
          ...
          {links.map((l) => {
            ...
            return (
              <Link key={l.to} to={l.to} onClick={onClose} title={collapsed ? label : ""}
                ...
                className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-sm transition-colors min-h-[44px] ${collapsed ? "justify-center" : ""} ${isActive(l) ? "bg-[#2563EB] text-white" : "text-slate-300 hover:bg-white/10 hover:text-white"} relative`}>
                <l.icon size={16} className="shrink-0" />
                {!collapsed && <span className="flex-1 truncate">{label}</span>}
                {!collapsed && l.accent && !isActive(l) && <span className="text-[9px] uppercase font-bold bg-[#2563EB] text-white px-1.5 py-0.5 rounded">{t("common.new")}</span>}
                {collapsed && (
